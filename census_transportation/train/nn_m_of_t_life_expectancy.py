...
def main(learn_rate=1e-3, epochs=10, out=True):
    ...
    dataset = torch.load(fp+"tensor_dataset/m_of_t_life_expectancy.pt", weights_only=True)
    ...
    torch.save(model.state_dict(), fp+"models/nn_m_of_t_life_expectancy.pth")
    if out:
        print("Saved model state to nn_m_of_t_life_expectancy.pth")
